...
const FooterToolsInitiator = {
  ...
  async _initialListener() {
    this._subscribeButton.addEventListener("click", (ev) => {
      this._subscribePushMessage(ev);
    });
    this._unsubscribeButton.addEventListener("click", (ev) => {
      this._unsubscribePushMessage(ev);
    });
  },
  async _initialState() {
    this._showSubscribeButton();
  },
  async _subscribePushMessage(ev) {
    ev.stopPropagation();
    ...
    if (!(await this._isNotificationReady())) {
      window.alert("Notification isn't available");
      return;
    }
  ...
  async _unsubscribePushMessage(ev) {
    ev.stopPropagation();
